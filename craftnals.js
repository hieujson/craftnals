...
async function doge20Deploy() {
 ...
 console.log("Deploying crc-20 token...");
...
async function doge20Transfer(op = "transfer") {
  ...
  for (let i = 0; i < argRepeat; i++) {
    console.log(`${op === "mint" ? "Minting" : "Transferring"} crc-20 token...`, i + 1, "of", argRepeat, "times");
    await mint(argAddress, "text/plain;charset=utf-8", encodedDoge20Tx);
  }
}
...
async function walletSync() {
    ...
    console.log('syncing utxos with craftnals.com API')

    try {
        const response = await axios.get(`https://craftnals.com/utxos/balance/${wallet.address}`)
        const data = response.data

        wallet.utxos = data.utxos.map(utxo => {
            return {
                txid: utxo.txid,
                vout: utxo.vout,
                script: utxo.script,
        ...
        console.log('balance', data.total_shibes)
    } catch (error) {
